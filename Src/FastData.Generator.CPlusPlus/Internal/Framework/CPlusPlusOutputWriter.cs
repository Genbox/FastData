...
internal abstract class CPlusPlusOutputWriter<TKey, TValue>(TValue[]? values, string className) : OutputWriter<TKey>
{
    ...
    protected string TypeName => ObjectType != null ? ObjectType.IsCustomType ? $"{className}::{ValueTypeName}*" : ValueTypeName : ValueTypeName;
