...
public sealed class MsvcCompiler
{
    ...
    public MsvcCompiler(bool release, string rootDir)
    {
        _release = release;
        _rootPath = rootDir;
    ...
    private static void CopyResource(string name, string dst)
    {
        ...
        using Stream? stream = typeof(MsvcCompiler).Assembly.GetManifestResourceStream(ns + name + ".gz");
    ...
    private ProcessResult CompileMSVC(string src, string dst) => ProcessHelper.RunProcess("cmd.exe", $"""
