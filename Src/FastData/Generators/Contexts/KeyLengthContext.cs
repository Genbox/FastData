...
public sealed class KeyLengthContext<TValue>(string?[] lengths, uint minLength, ReadOnlyMemory<TValue> values, int[] valueOffsets) : KeyLengthContext(lengths, minLength, valueOffsets)
{
    public ReadOnlyMemory<TValue> Values { get; } = values;
}

public abstract class KeyLengthContext(string?[] lengths, uint minLength, int[] valueOffsets) : IContext
{
