...
internal sealed record DefaultStringHash : IStringHash
{
    ...
    private DefaultStringHash(GeneratorEncoding encoding, bool ignoreCase = false)
    {
        _encoding = encoding;
    ...
    public Expression<StringHashFunc> GetExpression() => ExpressionHashBuilder.BuildOneByOne(352654597ul, Mixer, Avalanche, _encoding, _ignoreCase);

    internal static DefaultStringHash GetInstance(GeneratorEncoding enc, bool ignoreCase = false)
    {
        if (ignoreCase)
            return new DefaultStringHash(enc, true);
    ...
    private static BinaryExpression Mixer(Expression hash, Expression read) => ExclusiveOr(Add(Or(LeftShift(hash, Constant(5)), RightShift(hash, Constant(27))), hash), read);

    // 352654597 + (hash * 1566083941)
    private static BinaryExpression Avalanche(Expression hash) => Add(Constant(352654597ul), Multiply(hash, Constant(1566083941ul)));
}