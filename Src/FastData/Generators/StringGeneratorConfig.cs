...
{
    internal StringGeneratorConfig(Type structureType, uint itemCount, uint minLen, uint maxLen, bool ignoreCase, CharacterClass classes, GeneratorEncoding encoding, IEarlyExit[] earlyExits, string trimPrefix, string trimSuffix, bool typeReductionEnabled, StringHashInfo? hashInfo) : base(structureType.GetFriendlyName(), earlyExits, itemCount, typeReductionEnabled)
    {
