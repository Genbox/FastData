...
internal sealed partial class GPerfAnalyzer(int dataLength, StringKeyProperties props, GPerfAnalyzerConfig config, Simulator sim, ILogger<GPerfAnalyzer> logger) : IStringHashAnalyzer
{
    ...
    public IEnumerable<Candidate> GetCandidates(ReadOnlySpan<string> data)
    {
        ...
        if (props.LengthData.MinCharLength == 0)
            return [];
        ...
        int maxLen = props.LengthData.MaxCharLength;
        ...
        GPerfStringHash stringHash = new GPerfStringHash(table.Values, alphaInc, positions.OrderByDescending(x => x).ToArray(), props.LengthData.MinCharLength);
    ...
    private int[] GetPositions(ReadOnlySpan<string> data)
    {
        ...
        */
        ...
        int max = (int)Math.Min(props.LengthData.MaxCharLength - 1, config.MaxPositions - 1);
