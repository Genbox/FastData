...
The packable library under `Src/<Name>` targets `netstandard2.0`; tests, examples, and benchmarks target .NET 10. Keep library code and dependencies compatible with `netstandard2.0`. APIs available only in newer .NET versions belong in executable projects, not the library.
