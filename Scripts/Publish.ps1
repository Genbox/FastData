...
if ($env:PWSHG_KEY) {
    Write-Host -BackgroundColor $Color "Publish PowerShell to PowerShell Gallery"
    Publish-Module -Path "$PublishDir/Genbox.FastData/" -NuGetApiKey $env:PWSHG_KEY
} else {
    Write-Host -BackgroundColor Yellow "Skipping PowerShell publish: PWSHG_KEY not set."
}
...
if ($env:NUGET_KEY) {
    ...
    Write-Host -BackgroundColor Yellow "Skipping NuGet push: NUGET_KEY not set."
}